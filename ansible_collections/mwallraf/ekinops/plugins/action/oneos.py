...
class ActionModule(ActionNetworkModule):
    def run(self, tmp=None, task_vars=None):
        ...
        self._config_module = (
            True if module_name in ["oneos_config", "config"] else False
        )
        ...
        if persistent_connection == "network_cli":
            ...
            if any(provider.values()):
                display.warning(
                    "provider is unnecessary when using network_cli"
                    " and will be ignored"
                )
                del self._task.args["provider"]
        elif self._play_context.connection == "local":
            provider = load_provider(oneos_provider_spec, self._task.args)
            pc = copy.deepcopy(self._play_context)
            pc.connection = "ansible.netcommon.network_cli"
            pc.network_os = "mwallraf.ekinops.oneos"
            ...
            warnings.append(
                [
                    "connection local support for this module is deprecated"
                    " and will be removed in version 2.14, use connection %s"
                    % pc.connection
                ]
            )
        else:
